...
#define MYCILA_JSY_VERSION          "9.0.1"
...
namespace Mycila {
  ...
  class JSY {
    public:
      ...
      float getTHDi1(float phi) const { return sqrt(pow(cos(phi), 2) / pow(_powerFactor1, 2) - 1); }
      float getTHDi2(float phi) const { return sqrt(pow(cos(phi), 2) / pow(_powerFactor2, 2) - 1); }
