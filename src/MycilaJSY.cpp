...
bool Mycila::JSY::read(const uint8_t address) {
  ...
  switch (_model) {
    case MYCILA_JSY_MK_163: {
      ...
      // S = P / PF
      parsed._metrics[0].apparentPower = parsed._metrics[0].activePower == 0 ? 0 : parsed._metrics[0].activePower / parsed._metrics[0].powerFactor;
      // Q = sqrt(S^2 - P^2)
      ...
      // aggregate
      parsed._aggregate = parsed._metrics[0];
      break;
    }
    ...
    case MYCILA_JSY_MK_194: {
      ...
      // S = P / PF
      parsed._metrics[0].apparentPower = parsed._metrics[0].activePower == 0 ? 0 : parsed._metrics[0].activePower / parsed._metrics[0].powerFactor;
      parsed._metrics[1].apparentPower = parsed._metrics[1].activePower == 0 ? 0 : parsed._metrics[1].activePower / parsed._metrics[1].powerFactor;
      // Q = sqrt(S^2 - P^2)
      parsed._metrics[0].reactivePower = sqrt(parsed._metrics[0].apparentPower * parsed._metrics[0].apparentPower - parsed._metrics[0].activePower * parsed._metrics[0].activePower);
      ...
      // aggregate
      parsed._aggregate = parsed._metrics[0];
      parsed._aggregate += parsed._metrics[1];
      String s;
      break;
    }
